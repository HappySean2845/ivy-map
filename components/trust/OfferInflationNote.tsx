...
export default function OfferInflationNote() {
  ...
  return (
    <div className=" border border-ink/15 bg-paper/70 px-3 py-2 text-sm leading-relaxed">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
      ...
      >
        ...
        <span className="shrink-0 text-xs text-ink/60">{open ? '收起' : '展开'}</span>
      </button>

      {open && (
        <div className="mt-2 border-t border-ink/15 pt-2">
          <p className="text-[13px] leading-relaxed text-ink/60">{BODY}</p>
          <p className="mt-1.5 text-[11px] leading-relaxed text-ink/60">{CAVEAT}</p>
        </div>
      )}
    </div>
  )
