...
export function UniversityRequirements({ profile }: { profile: UniversityRequirementProfile }) {
  return (
    <section className="mt-12">
      <p className="label text-ink/40">录取要求与生活环境</p>
      ...
      <div className="mt-6 grid gap-10 lg:grid-cols-[minmax(0,1.45fr)_minmax(17rem,0.75fr)] lg:gap-14">
          ...
          <dl className="mt-5 border-t border-ink">
            {REQUIREMENT_KEYS.map((key) => {
              ...
              return (
                <div
                  key={key}
                  className="grid gap-1 border-b border-ink/15 py-3 sm:grid-cols-[5rem_7.5rem_1fr] sm:gap-3"
                >
        ...
        <aside className="border-t border-ink pt-4 lg:border-t-0 lg:border-l lg:pl-8">
          <p className="label text-ink/40">落地环境</p>
